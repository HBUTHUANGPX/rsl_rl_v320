...
class MultiTeacherDistillationRolloutStorage(RolloutStorage):
    ...
    class Transition():
        ...
        def __init__(self) -> None:
            ...
            self.student_action_mean: torch.Tensor | None = None
            self.student_action_sigma: torch.Tensor | None = None
            self.teacher_action_mean: torch.Tensor | None = None
            self.teacher_action_sigma: torch.Tensor | None = None
    ...
    def __init__(
        ...
        print(f"Initializing {self.__class__.__name__} with {num_envs} envs and {num_transitions_per_env} transitions per env.")
        ...
        if training_type == "rl":
            ...
            self.student_mu = torch.zeros(num_transitions_per_env, num_envs, *actions_shape, device=self.device)
            self.student_sigma = torch.zeros(num_transitions_per_env, num_envs, *actions_shape, device=self.device)
    ...
    def add_transition(self, transition: Transition) -> None:
        ...
        if self.training_type == "rl":
            ...
            self.student_mu[self.step].copy_(transition.student_action_mean)
            self.student_sigma[self.step].copy_(transition.student_action_sigma)
    ...
    def mini_batch_generator(self, num_mini_batches: int, num_epochs: int = 8) -> Generator:
        ...
        old_student_mu = self.student_mu.flatten(0, 1)
        old_student_sigma = self.student_sigma.flatten(0, 1)
        old_teacher_mu = self.teacher_mu.flatten(0, 1)
        old_teacher_sigma = self.teacher_sigma.flatten(0, 1)
        ...
        for epoch in range(num_epochs):
            for i in range(num_mini_batches):
                ...
                old_student_mu_batch = old_student_mu[batch_idx]
                old_student_sigma_batch = old_student_sigma[batch_idx]
                old_teacher_mu_batch = old_teacher_mu[batch_idx]
                old_teacher_sigma_batch = old_teacher_sigma[batch_idx]
