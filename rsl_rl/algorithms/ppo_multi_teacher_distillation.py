...
class PPO_Distil:
    """PPO with integrated distillation using DKL loss (instead of MSE BC loss).

    This algorithm combines standard PPO with online distillation during reinforcement learning.
    ...
    def compute_returns(self, obs: TensorDict) -> None:
        ...
        last_values = self.policy.evaluate_values(obs).detach()
    ...
    def update(self) -> dict[str, float]:
        ...
        for (
            ...
            if hasattr(self.policy, "beta_kl"):
                _, CVAE_kl = self.policy.act_inference(obs_batch, need_kl=True)
            ...
            value_batch = self.policy.evaluate_values(obs_batch, masks=masks_batch, hidden_state=hidden_states_batch[1])
